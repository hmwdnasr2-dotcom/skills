 ...
 *  - content: unknown[] → passed directly as ContentBlockParam[] (multi-modal turns)
 */
function toAnthropicMessages(messages: Message[]): Anthropic.MessageParam[] {
  ...
  for (const m of messages) {
    if (m.role === 'tool') {
      ...
      // Multi-modal user message — content is already formatted as content blocks.
      result.push({
        role: m.role as 'user' | 'assistant',
        content: m.content as Anthropic.ContentBlockParam[],
      });
    } else {
      result.push({ role: m.role as 'user' | 'assistant', content: m.content as string });
    }
  }
